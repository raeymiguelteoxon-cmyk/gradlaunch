...
function NavBar({ page, setPage }) {
  return (
    <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:100, background:"rgba(8,11,18,0.92)", backdropFilter:"blur(16px)", borderBottom:`1px solid ${T.border}`, padding:"0 2rem" }}>
...
function Home() {
  ...
  const runSearch = async () => {
    ...
    try {
      ...
      const jobs = buildJobLinks(data.profile, filters);
      clearInterval(mt);
      setResults({ profile: data.profile, jobs });
      setStage("results");
    } catch (e) {
      clearInterval(mt);
            ...
            >
              ...
              {file ? (
                <>
                  ...
                  <p style={{ color:T.textMid, fontSize:13 }}>Resume loaded · Click to change file</p>
                </>
              ) : (
                <>
                  ...
                  <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                    {opts.map(o => (
                      <button key={o} onClick={() => setFilters(f => ({ ...f, [key]:o }))}
                        style={{ padding:"8px 16px", borderRadius:8, border:`1.5px solid ${filters[key]===o?ac:T.border}`, background:filters[key]===o?ac+"18":"transparent", color:filters[key]===o?ac:T.textMid, fontSize:12, fontWeight:600, cursor:"pointer", transition:"all 0.18s", fontFamily:"'Syne',sans-serif" }}>
                        {o}
                      </button>
                    ))}
                  </div>
      ...
      {stage === "results" && results && (
        <section style={{ minHeight:"100vh", padding:"88px 2rem 60px", animation:"fadeUp 0.5s ease" }}>
          <div style={{ maxWidth:1100, margin:"0 auto" }}>
            ...
            {/* Profile strip */}
            <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:16, padding:"18px 24px", marginBottom:24, display:"flex", alignItems:"center", gap:18, flexWrap:"wrap" }}>
              ...
              <Btn variant="ghost" onClick={reset} style={{ fontSize:11, padding:"7px 14px", flexShrink:0 }}>↺ New Search</Btn>
            </div>
            ...
            <div style={{ background:T.accentGlow, border:`1px solid ${T.accent}33`, borderRadius:14, padding:"18px 22px", marginBottom:28, display:"flex", gap:14 }}>
              ...
              <div>
                ...
                <p style={{ color:T.text, fontSize:14, lineHeight:1.75 }}>{results.profile?.summary}</p>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))", gap:16 }}>
              {results.jobs?.map((job, i) => {
                ...
                const scoreColor = job.matchScore >= 90 ? T.green : job.matchScore >= 80 ? T.gold : T.accent;
                return (
                  <div key={i}
                    ...
                    <div style={{ padding:22 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:14 }}>
                        <div style={{ flex:1 }}>
                          ...
                          <h3 style={{ fontFamily:"'Instrument Serif',serif", fontSize:17, color:T.text, lineHeight:1.3 }}>{job.title}</h3>
                        </div>
                        <div style={{ textAlign:"right", flexShrink:0, marginLeft:12 }}>
                          <div style={{ fontFamily:"'DM Mono',monospace", fontSize:22, fontWeight:500, color:scoreColor }}>{job.matchScore}%</div>
                      ...
                      {isOpen && (
                        <div style={{ marginTop:16, animation:"fadeUp 0.2s ease" }}>
                          <div style={{ background:T.surfaceHigh, border:`1px solid ${T.border}`, borderRadius:10, padding:14, marginBottom:14 }}>
      ...
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"60px 2rem" }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap", justifyContent:"center", marginBottom:48 }}>
          {CAREER_PATHS.map((cp,i) => (
            <button key={cp.degree} onClick={() => setActive(i)}
              style={{ padding:"9px 18px", borderRadius:10, border:`1.5px solid ${active===i?T.accent:T.border}`, background:active===i?T.accentGlow:T.surface, color:active===i?T.accent:T.textMid, fontSize:12, fontWeight:700, cursor:"pointer", transition:"all 0.2s", fontFamily:"'Syne',sans-serif" }}>
              {cp.icon} {cp.degree}
            </button>
          ))}
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
          ...
          <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:16, padding:28 }}>
            ...
            {p.certs.map(c => <div key={c} style={{ display:"flex", gap:10, color:T.textMid, fontSize:13, marginBottom:10 }}><span style={{ color:T.green }}>✓</span>{c}</div>)}
          </div>
          <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:16, padding:28 }}>
